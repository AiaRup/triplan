    ...
    navOnLogin = () => {
      ...
      const authNav = this.state.authenticated ? (
        <ul className="auth-nav nav navbar-nav navbar-right">
          ...
          <li className="nav-item nav-link"><button className="btn btn-sm btn-secondary my-2 my-sm-0 btn-logout" type="button" onClick={() => this.props.auth.logout()}>Log Out</button></li>
        </ul>
      ) : (
        <ul className="auth-nav nav navbar-nav navbar-right">
